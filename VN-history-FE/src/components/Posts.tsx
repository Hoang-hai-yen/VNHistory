...
const Posts: React.FC = () => {
  ...
  const renderStatusTag = (status: string) => {
    ...
    return <span className={`status-tag status-${statusMap[status]}`}>{status}</span>;
  };
    ...
    <div className="posts-page">
      ...
      <div className="posts-toolbar">
        <div className="tabs-container">
          <button className={`tab-link ${activeTab === 'TẤT CẢ' ? 'active' : ''}`} onClick={() => setActiveTab('TẤT CẢ')}>TẤT CẢ (1,284)</button>
          <button className={`tab-link ${activeTab === 'XUẤT BẢN' ? 'active' : ''}`} onClick={() => setActiveTab('XUẤT BẢN')}>XUẤT BẢN (1,102)</button>
          <button className={`tab-link ${activeTab === 'CHỜ DUYỆT' ? 'active' : ''}`} onClick={() => setActiveTab('CHỜ DUYỆT')}>CHỜ DUYỆT (47)</button>
          <button className={`tab-link ${activeTab === 'BẢN NHÁP' ? 'active' : ''}`} onClick={() => setActiveTab('BẢN NHÁP')}>BẢN NHÁP (135)</button>
        </div>
          ...
          <tbody>
            {contentData.map(item => (
              <tr key={item.id}>
                ...
                <td><span className={`type-badge ${item.type === 'SỰ KIỆN' ? 'type-event' : 'type-person'}`}>{item.type}</span></td>
                <td>{item.dynasty}</td>
                <td>{item.assignee}</td>
                <td>{item.date}</td>
